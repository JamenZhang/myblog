    ...
    function putInArticle(data) {
        $('.jamen-article-top').html('');
        $('.jamen-article-footer').html('');
        var articleTop = $('<article-top><div class="article-title">' +
            ...
            '不举步，越不守栅栏，不迈腿，登不上高山。<span class="article-i-say-me">--- 张俊民</span>' +
            '</div>');
        $('.jamen-article-top').append(articleTop);
        ...
        var articleFooter = $('<div class="end-logo">' +
            ...
            '<img src="https://qiniu.iifxs.xyz/1546233109.png">' +
            '</p>' +
            '<p class="show-weixin-pic">欢迎关注我的微信公众号：baiwenhei</p>' +
            ...
            '<li><strong>版权声明：</strong> 本博客所有文章采用<span id="copyRightFooter"><a href="https://creativecommons.org/licenses/by/3.0/cn/" target="_blank"> CC BY 3.0 CN协议</a></span>进行许可。转载请署名作者且注明文章出处。</li>' +
            '</ul>' +
            '</div>' +
            ...
            '</span>' +
            '</div>');
        $('.jamen-article-footer').append(articleFooter);
        ...
        $('.other').append($('<div class="social-share" data-initialized="true" data-url="https://www.iifxs.xyz/article/' + data.articleId  + '"  data-title="' + data.articleTitle + '">' +
    ...
    $.ajax({
        ...
        success:function (data) {
            if(data.status == "200"){
                ...
                $('.content').html('');
                var error = $('<div class="article"><div class="jamen-article-top"><div class="error">' +
                    '<img src="https://qiniu.iifxs.xyz/blog/static/img/register_success.jpg">' +
                    '<p>没有找到这篇文章哦</p>' +
                    '<p>可能不小心被博主手残删掉了吧</p>' +
                    '<div class="row">' +
                    '<a href="/">返回首页</a>' +
